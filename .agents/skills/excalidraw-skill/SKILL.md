...
description: Use when the user explicitly requests Excalidraw or an editable .excalidraw file, or wants a casual, hand-drawn, whiteboard-style flowchart, architecture sketch, mind map, sequence diagram, or workflow with optional PNG/SVG export. Prefer drawio-skill for polished, precise, strict UML, branded vendor-icon, or high-fidelity diagrams.
license: MIT
compatibility: Requires curl and network access for Kroki SVG export; local PNG/SVG export optionally requires Node.js, Firefox, Playwright, and excalidraw-brute-export-cli.
metadata: {"version":"1.3.0","source":"Agents365-ai/excalidraw-skill@00606e9fcb072e9644cbfbb3d49a9dafe8b98c25"}
---
...
**Explicit triggers:** user requests "Excalidraw", an editable `.excalidraw` file, a whiteboard sketch, or a hand-drawn/casual diagram.

**Proactive triggers (only when a casual, hand-drawn, or whiteboard style fits):**
...
- Polished, precise diagrams, strict UML, branded vendor icons, or high-fidelity replication → **drawio-skill**.
...
```json
{
  "type": "excalidraw",
  "version": 2,
  "source": "https://github.com/zixuandai0217/MySkills",
  "elements": [],
...
python3 <skill-dir>/scripts/excalidraw_lib.py merge scene.excalidraw \
...
> **Need copy-paste templates or the full property/arrowhead catalogue?** Read `<skill-dir>/references/schema-reference.md` — complete element templates (component+label, bound arrow, arrow label, swimlane zone, mind-map connector) and every verified property value.
...
1. **Render to PNG** (the image must be viewable — PNG, not SVG, even if the user ultimately wants SVG):
   ...
   ```
   Use the host's image-viewing capability to inspect `/tmp/check.png`. *Visual audit needs the local CLI; with Kroki-only (SVG), fall back to the structural checks below.*
2. **Audit the image:**
